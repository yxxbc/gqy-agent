...
| `tools` | 回合可用工具白名单。回合以主人身份运行，别加 `run_command` 这类 | 搜索、看图、记忆、知识库等 |
...
| `split_paragraphs` / `max_bubbles` | 按空行拆成多条发送及条数上限 | `true` / `6` |
...
改 `imessage_bridge.py` 后，脚本在空闲时（没有进行中的回合）自动用新代码重载。改出语法错误时不重载，日志里会报错并继续跑旧代码。
...
- 支持私聊文字、收图、发表情包和图库图片。群聊、点按回应忽略。她新生成的图片暂不发送。
