...
- 增强自身认知与环境感知能力：网关守护进程（daemon）启动时将工作目录明确绑定至用户家目录，避免从源码目录启动时误将仓库路径作为默认工作目录；WebUI 会话在缺少客户端显式路径时默认以用户家目录作为工作目录。系统提示词 `<host-environment>` 明确区分宿主用户家目录（`user_home`）与 `gqy_home`，每轮瞬态 `<runtime>` 块中准确感知并注入当前接入客户端（`client="webui"`、`client="cli"` 等），让模型掌握自身所处环境与交互通道
- 优化工具认知与前缀容错：明确告知常用工具（直接调用的 `mcp_gqy_*`）与惰性挂载工具（通过 `call_mcp_tool` 调用的工具）的区别；工具注册中心支持自动识别并剥离 `mcp_gqy_`、`mcp_`、`gqy:` 等调用别名前缀，对误将宿主原生工具（如 `run_command`、`view_file`）传入 `load_tools` 的情况给出明确引导提示，杜绝模型误判为「unknown tool or script」

### Removed

- 彻底去除 Antigravity (agy) 预热逻辑：移除后台预热进程拉起、空闲回收与相关配置选项（`warm_idle_seconds` 等），彻底杜绝孤儿 agy 进程残留与多余的内存及 MCP 连接占用，运行转为纯按需拉起、即用即走
