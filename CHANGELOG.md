...
- 后台任务条会如实显示「预热进程」：一轮结束后提前拉起的 agy（占一个进程 + 一条 MCP 桥）还晾在后台时，条上多一行「预热进程 · 剩 X」，到期自动消失——以前它只出现在活动监视器里，没人知道那是什么；Antigravity 设置里预热保留时长的说明也写清了成本与回收规则
...
- daemon 退出或重启时主动收掉晾着的预热进程：它挂在内存池里、进程退出不走析构，以前只能等 agy 自己退（不保证），极端情况下重启后会留下孤儿 agy
