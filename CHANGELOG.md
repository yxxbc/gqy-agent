...
- 内置供应商多一个「Cline 中转」：借本机已登录的 `cline`（`cline auth`）当模型后端，和 Claude Code / Antigravity / Codex 三条本机 CLI 中转线同一形态，默认未启用。模型清单直接问 cline 本体要（与它自己的模型选择器同源），也能手工添加；思考档位 none / low / medium / high / xhigh 映射到 `--thinking`；人设整体替换，顾清影的工具经 MCP 桥挂进去（你自己的 MCP 服务器不受影响）。终端配置菜单、WebUI 设置和首次引导里都能开和调。会话续传默认开启：第一轮全量重放并认下会话，之后只发增量；发现 cline 没按续传认下就自动退回全量重放——多花 token，不静默丢历史
...
- WebUI 输入框换了样式：模型选择挪进框内左下角（附件按钮旁），上下文用量圆环挪到发送键左边；外框改为更圆的细描边，输入时描边高亮。框下只留运行状态、速度和累计，手机上少占一行
...
- WebUI 设置里的供应商头像不再清一色字母：认得出的品牌（Anthropic / Claude、OpenAI / Codex、Gemini、DeepSeek、Cline、Kimi、豆包、Ollama、OpenRouter、Mistral 等 24 个）显示品牌图标，供应商卡片与模型池成员两处都有；认不出的仍回落到字母 + 哈希底色。图标内嵌在前端，离线可用
