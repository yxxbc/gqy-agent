...
- 通讯平台接入层：新增通用连接器协议。平台的收发 I/O 可以放在任意语言写的连接器里，连接器带口令连上本机 WebSocket（`/api/connector/ws?platform=<平台>`）就能和 gqy 收发消息；以后加平台不用改 gqy、不用重新编译。iMessage 是第一个用它的平台
- iMessage 从独立桥接变成 gqy 的原生平台：会话与话题、指令、拆气泡与打字节奏、模型、记忆、引用与点按回应、表情包 / 图库 / 生成的图、语音都由 gqy 处理，桥接只负责读 `chat.db` 收消息和经「信息」发出。长按她的某条消息回复，她知道你在回哪一句；你点的 ❤️ 等回应她也看得到，回应本身不会让她回一条
- iMessage 里她能把刚画的图直接发到你手机上（只发生图插件输出目录里的图片）
- iMessage 的指令和 QQ 走同一套机制：`/new` 开新话题、`/topics` 看话题、`/topic 2` 切换，`/model` 查看和切换模型（只对这个聊天生效），`/pause` / `/resume` 暂停和恢复，`/help` 看说明。指令由 gqy 直接处理，不经过模型
- WebUI 的「控制台 → 平台」多一个 iMessage：连接状态、连接器地址与口令、联系人名单（同一个人的多个账号合成一个，你标成主人的那个算主人）、回复设置（最多拆几条、气泡间停顿、是否允许写记忆、本人能不能用宿主工具）
- 终端配置菜单的通讯平台里多一项 iMessage：开关、生成新口令、看联系人个数，其余指去 WebUI 或配置文件改
- 资料卡点赞（插件 `qq_profile_like`，默认开）：对方说「给我点个赞」，她会给**请求的人本人**点资料卡赞，不能让她去给别人刷赞。主人号在哪都能要；其他人看插件里的两份名单——私聊按 QQ 号、群聊按群号，不满足条件的会话根本没有这个工具。每人每天最多 10 个（QQ 普通账号的上限），本地先记账挡住
- 群聊黑名单：管理员说「以后别理他」「先晾他一天」她就记下来。被拉黑的人说的话照常进群聊记录，但她不再回他——@、关键词、接话、主动插话都不触发，也不贴「处理中」表情。默认只在当前群，也能设成所有群，可设时长、到期自动解除
- 主动私聊（插件 `qq_private_initiative`，默认关闭）：私聊里她回复之后安静满一段时间，会问一次辅助模型「要不要、什么时候主动找对方」，有合适的理由才排计划；到点她用自己的口吻发一条，不说自己是定时的。只对管理员与私聊白名单，睡眠时段不发，每人每天 1 次（可调）
- 回复贴表情（插件 `qq_reply_reaction`，默认开）：群里她回复一条消息时按概率（默认 30%）在那条消息上贴一个常驻表情（爱心 / 赞 / OK），表情和概率在「平台 → QQ → 通用 → QQ 插件」里改。和主动插话时那个会自动撤掉的「处理中」表情是两回事
- 主人 QQ 号：在配置里写 `platforms.qq.owner_users`（WebUI「平台 → QQ → 通用」），这些号在私聊里按主人本人对待——读全部记忆，写下的记忆算主人自己的（来源标为「主人 QQ」），并带上用户资料；私聊用的人格与终端相同时，记忆与终端、网页端打通。群聊里不生效
- 内置供应商多一个「Cline 中转」：借本机已登录的 `cline`（`cline auth`）当模型后端，和 Claude Code / Antigravity / Codex 三条本机 CLI 中转线同一形态，默认未启用。模型清单直接问 cline 本体要（与它自己的模型选择器同源），**列出时就把上下文窗口一并带回、激活模型时自动填好**，也能手工添加；思考档位 none / low / medium / high / xhigh 映射到 `--thinking`；人设整体替换，顾清影的工具经 MCP 桥挂进去（你自己的 MCP 服务器不受影响）。终端配置菜单、WebUI 设置和首次引导里都能开和调。会话续传默认开启：第一轮全量重放并认下会话，之后只发增量；发现 cline 没按续传认下就自动退回全量重放——多花 token，不静默丢历史
- WebUI 上下文弹窗新增「上下文走势」：每个完成回合结束时的上下文占用画成曲线，并按最近几轮的增长速度估算还有几轮到自动压缩。「占用最多的 5 条」列表随之撤下
- 普通模式的空白页变成一张「今天」的花笺：称呼你的名字、把当天天气写进句子里，附上相识第 N 天与「上次你说」。天气城市在信纸角上填一次（按账号存），由本机 daemon 代取（Open-Meteo，免 key，同城 15 分钟内复用），带伞 / 穿搭这类建议在前端按规则算
- 账号页新增「头像」卡：你和她的头像都能上传，拖动取景、滚轮或滑块缩放，对话里头像的尺寸也能调；预览实时、按账号保存，她的取景按人格各记一份
- WebUI 侧栏分成「两间屋子」：左上角「普通 / 开发」开关只列当前模式的会话，新对话也落在当前模式，刷新后回到原来那间。普通模式的侧栏是「她的房间」（看板、按时辰的问候、相识天数），会话收进「信匣」浮层——Ctrl/Cmd+K 呼出，可搜索、方向键选择；开发模式下它是命令面板
- 开发模式有了自己的门面：底部 tmux 风格状态栏（会话＝窗口、Alt+1…9 跳转、右侧模型与上下文读数）和切入时的 HUD 启动序列；普通模式进入时有粒子聚拢、聊天背后常驻落梅——系统「减少动效」或页面在后台时都会停
- 输入框草稿按会话各存一份：切会话、切模式、刷新都不丢；只存文字，附件仍随会话清
- 她能给你「寄一封信」了（WebUI 专属）：网页会话里她想写的时候，对话里落下一只封着火漆的信封——点开飞到屏幕中央，拆印、翻盖、花笺铺开、字一行行浮上来、朱印按下；信封上的称呼与蜡印跟着人格名字走，落款是当天。这件工具只在普通模式的网页会话里注册，终端与 QQ / iMessage 没有

### Changed

- **破坏性** iMessage 改用 gqy 的原生平台：配置从桥接自己的 `config.json` 搬到 gqy 配置的 `platforms.connectors.imessage`（联系人、口令、回复设置），桥接脚本只剩收发，`scripts/imessage/` 路径与启动器不变，macOS 的完全磁盘访问与自动化授权不用重做。旧会话（`imessage-<联系人>`、`imessage-<联系人>-N`）原名沿用，聊天历史不丢。改完 `gqy reload` 生效
...
- WebUI 控制台新增「平台」页：左侧是通讯平台列表，QQ 的连接与设置、消息记录、群聊管理都在这里（原来分散在设置页的「QQ 平台」和控制台的「QQ 消息记录」「群管」三处）。以后接入的平台也放在这一页。旧的 `#console/qq` 等链接会自动跳到新位置
- WebUI 的 QQ 平台页分成「通用 / 私聊 / 群聊」三个页签，一个设置项落在哪一页按它的配置键归位，不用在长页面里上下找；会话专属配置按类型列在对应页签
- 主人 QQ 号不用再在管理员名单里重复填一遍：静态管理员判断两份名单都认，主人号排在管理员前面，主动联系时优先
- 资料卡点赞与回复贴表情从散落的私聊 / 群聊设置项收进 QQ 插件卡片，各有独立的开关与参数
- WebUI 左下角去掉了单独的设置按钮，设置从「控制台 → 设置」进入
- WebUI 部分图标换成新版画法（删除、刷新、图片、文件夹等），各页面用的是同一套图标
- WebUI 看板里的操作提示改用和聊天界面相同的提示条
- WebUI 的小字统一到 11 / 12 / 13px 三档：会话列表的时间和标签、待办状态、提问卡片、artifact 标题里原来 9–12.5px 的小字略微放大，更容易看清
- WebUI 输入框换了样式：上下文用量圆环挪到发送键左边；外框改为更圆的细描边，输入时描边高亮；模型切换从框内挪到输入框下方那行的左端，手机上也不再缩成光秃秃一个箭头。模型菜单也换了样：按供应商分节、节头带品牌图标、点开才铺模型，顶部有过滤框（敲 `kimi`、`cline-pass` 这类关键词直接筛），十几个供应商几十个模型不用再从头翻
- WebUI「设置 → 全局」按主题分成顶部分页：工具 / 对话 / 记忆 / 语音与通知 / 账号与平台，不用再在八屏长的页面里上下翻找
- iMessage 回复的气泡更少、更均匀：相关的句子放在同一条里，内容不减；段落超过 6 条上限时均衡地合并，不再全挤进最后一条长消息；多条之间按长度停顿一下再发，像在打字（`bubble_pause_seconds` 可调，`0` 为连发）
- WebUI 设置里的供应商头像不再清一色字母：认得出的品牌（Anthropic / Claude、OpenAI / Codex、Gemini、DeepSeek、Cline、Kimi、豆包、Ollama、OpenRouter、Mistral 等 24 个）显示品牌图标，供应商卡片、模型池成员与模型菜单的供应商分节三处都有；认不出的仍回落到字母 + 哈希底色。图标内嵌在前端，离线可用
- Cline 的「cline 供应商 id」从手填输入框升级为可搜索候选：拉的是本机 cline 自带的全部供应商并标出各家模型数（如 `cline` 317 个、`cline-pass` 订阅 18 个），边打边筛、也可手填候选外的值。说明里写清了这个 id 同时决定 `-P` 与模型目录——订阅档（Pass / 免费 / 各家 plan）就是靠它区分
- 普通模式首页重做：起手语换成「陪我说说话 / 给我讲个睡前故事 / 发表情包打个招呼吧 / 写首小诗给我」，开发模式另有「看看仓库结构 / 排查一个报错 / 补一组测试 / 解释一段代码」；空白页改成竖排问候与立绘淡入，输入框有草稿时描边换成流动的墨色
- 对话里的头像统一走同一套取景框（描边、取景、尺寸都跟账号设置走），她的名字改用衬线发金色，气泡圆角更大一档
- 输入框边的上下文圆环改成小表盘：细外圈 + 按占用比例填的扇形，原来的细弧在用量低时看着像「加载中」
- 她更清楚自己在哪、你从哪来：daemon 不管从哪个目录启动，工作目录都是你的家目录（以前从源码目录启动，她会把 gqy-agent 仓库当成「自己的家」，问她清理家目录就去翻源码）；网页会话没有指定工作目录时也落在家目录。系统提示词里说清 `gqy_home` 才是她自己的家、GQY 源码只是她运行的软件；每轮的运行时戳带上工作目录和你是从哪条路来的——`webui`、`cli`、`subagent`，平台写到具体平台与会话类型（如 `qq/group`、`imessage/private`），历史里旧的戳与之不符时以最新的为准
- 工具调用报错对所有供应商都说实话：拼错的工具名给近似候选（如 `read_file` → `read_files`），`mcp_gqy_` / `mcp_` / `gqy:` 这类前缀自动剥掉再找。「这是你宿主 CLI 自带的原生工具，直接调用即可」这句只在 Claude Code / Antigravity / Codex / Cline 这类中转后端的工具桥上出现——直连 API 的供应商本来就没有原生工具，不会再被它误导着绕圈。Antigravity 下常驻工具与懒加载工具各怎么调也写清了

### Removed

- 去掉 Antigravity（agy）预热：不再在一轮结束后提前拉起下一轮的 agy 晾着，杜绝残留的 agy 进程和多占的内存、MCP 连接，改为每轮按需拉起。WebUI 任务条上的「预热进程」一行随之消失；旧配置里的 `warm_idle_seconds` 会被忽略

### Fixed

- 信匣 / 命令面板里会话卡片的「…」菜单不再被后面的卡片盖住或被面板裁掉；在别的卡片上点「清空对话」，清的是那张卡片的会话（确认框写着它的名字），当前正在看的会话不受影响。以前清的总是当前会话
- 终端新手引导「认识你」一步的示例文字还是上游项目的「我叫 shorin，是你的 Master」，改成了中性的示例
- 在 WebUI 看板里操作时如果登录已过期，现在会直接回到登录页，不再只弹一条报错
...
- 终端 TUI 时间线文件类工具输出点击：读文件、搜文件、补丁编辑等工具在时间线上注册详情块，点击可展开查看路径、参数或 diff，输出为空时仍可展开查看操作目标，无内容点击时给出轻量提示反馈
- 终端 TUI 输入框多行换行点击定位：修复输入框自动折行后鼠标点击文字光标跳回句尾及底栏重复绘制变成两行的问题
- 终端 TUI 行内代码渲染：修复 Markdown 解析器处理连续反引号与链接标签嵌套时的样式解析问题
- 终端 TUI 交互与视觉优化：空会话中 `/goal` 保持底栏稳定并在历史中留痕；`/usage` 改为 Esc 可关闭的全屏覆盖层；欢迎横幅吉祥物支持多阶宽度自适应与碎碎念展示；空会话隐藏未发言时的上下文占用；`/config` 顶层铺开配置子项直达菜单
- `gqy export` 补上原来漏掉的路径：主题库（`config/webui-themes/`）按 Core 导出，iMessage 连接器的配置与联系人名单随 `--platforms` 导出（名单里有手机号与邮箱）；桥接的读取水位只对这台机器的 `chat.db` 有效，不导出
- 回复中途失败时不再只丢一句英文报错：现在会说明原因（上游内容策略拦截、限流、上游卡死被终止等），已经写出的半截内容旁边多一个「继续」，点一下她就接着说完
- 回合失败时的中文原因说明（内容策略拦截、限流、连不上上游等）此前实际未生效：服务端认清失败类型的取法不对，从未认出过任何一类，提示条只能显示英文原文；现在按错误链里真正挂着的失败类型给出中文说明
...
- 连接器必须带口令才能连上，空口令一律拒绝（不认「来自本机」：沙盒里的会话也能连本机端口）；口令比对走 SHA-256 摘要，防时序与长度探测
- iMessage 里标成主人的联系人，默认不能在聊天窗口里让她跑命令、读写文件（要开 `owner_host_tools` 才给），手机丢了或账号被盗时不会变成操作电脑的后门；记忆仍与终端共享
- 群里管理员不再能读到主人的私密记忆：以前管理员的特权记忆读取在群聊里也生效，主人的私聊记忆可能被注入到群里所有人都看得见的回复里。现在特权读取只发生在私聊，群里管理员只看到自己的和公开的记忆

## [0.7.0] - 2026-09-24

顾清影有了英文名 Selene，内置人格改为面向所有用户的版本。终端界面换了配色并能适配浅色终端，安装以 Nix 为主，一键安装脚本有了进度界面。内置知识库换成顾清影自己的使用手册，她会纠正记忆、会在聊完后复盘。
...
- 聊后复盘：终端或网页端的对话安静一段时间后，顾清影回顾这段对话，最多记 3 条改进要点，下次聊天时参考。等待时长由 `plugins.memory.review_idle_seconds` 控制（默认 900 秒，0 关闭）。WebUI 记忆页新增「复盘」页签，只读
...
- TUI 斜杠命令能用方向键选：输入 `/` 弹出候选后，↑↓ 移动，Tab 填入，Enter 直接执行选中的命令，Esc 关掉（再打字会重新弹出）。需要必填参数的命令按 Enter 只填入，等你补参数
- `/config` 补齐了约 30 项原来只能在 WebUI 改的设置（上下文与压缩、工具超时与命令黑名单、沙盒目录、缓存、通知、成员账号、记忆参数），「全局参数设置」按显示 / 工具 / 上下文 / 缓存 / 通知 / 成员账号分组。`/config <分组>` 直接打开那一组，打 `/config ` 后候选里会列出各分组

### Changed

- 协议改为 [PolyForm Noncommercial 1.0.0](LICENSE)：个人、学习研究、非营利用途可以自由使用、修改和分享，禁止商用。本项目源自 miyu-agent，原作者的 MIT 声明保留在 `LICENSE-MIT`。0.6.0 及更早的版本仍按 MIT 发布，不受影响
...
- **破坏性**：打开 `gqy` 默认是新会话，不再自动回到上次关掉的对话。要接着上次聊用 `gqy -c`（开发模式 `gqy dev -c`），`gqy --session <名字>` 直接打开指定会话。上次那条还一句没说过的话会直接复用，反复开关不会攒一堆空会话。「回放轮数」设置改为只在回到上次会话时生效
- 安装改为以 Nix 为主（`nix profile install github:yxxbc/gqy-agent/gqy`），直接下载云端编译好的包；没有 Nix 或 Intel Mac 继续用 `install.sh`。用 `install.sh` 装过的，`nix/migrate.sh` 可以一键换到 Nix 版
...
- TUI 开屏换成欢迎框：左边是吉祥物，右边是欢迎语、当前目录、模式与一行提示，框下列出最近几个会话。原来的大号艺术字、星空和扫光动画撤掉，开屏不再持续重画
- 新配置项 `display.mascot`：开屏吉祥物可选顾清影立绘（默认；kitty 下显示原图，其他终端用字符画）、黑猫字符画、自定义（`config/banner.txt`）或不画。WebUI 设置页和 `/config` 都能改
- TUI 输入框改为淡灰色圆角框，提示符是模式主色的 `❯`（普通蓝、开发酒红）；你发出的消息在对话记录里显示为 `❯ 消息` 加整行淡底色，不再是左侧竖条。底栏移到框下方。拖选复制时框线和 `❯` 不会进剪贴板
- TUI 思考过程改为暗色斜体（原为亮绿），等待动画改用主色
- `gqy pm` 不再内置默认包索引：原来写死的官方索引仓库实际并不存在，按包名安装和搜索都会失败。现在需要先 `gqy pm tap add owner/repo` 添加索引；直接按 `owner/repo`、GitHub 地址或本地目录安装不受影响。之前存下的那个失效地址会被自动忽略
- 用 Antigravity（agy）时省下大量 token：原来每次模型调用都带着全部 84 个工具的完整说明（约 1.8 万 token），现在只常驻约 22 个常用工具，其余用到时再加载。自己的脚本或 MCP 工具要常驻，加到设置里「额外常驻的桥工具」（`plugins.antigravity.gqy_tools_eager_extra`）
...
- 用 Antigravity（agy）时 token 用量被严重高估：原来每轮记的是整个 agy 会话的累计值，底栏 Σ 和用量日志都被前面几轮反复计入。现在按本轮每次模型调用的实际用量求和
- `gqy --help` 顶部显示的是一段写给开发者的内部说明，现在只显示「顾清影 AI 助手」
- 重开会话回放历史时，工具步骤的耗时偶尔多算零点几秒，现在显示的就是当时记录的时长

## [0.6.0] - 2026-09-17

首个以 GQY / 顾清影名义发布的版本，基于 miyu-agent 0.6.0 深度重构与二次开发。上游 0.6.0 带来的新手引导、默认全屏 TUI、WebUI 时间线、stdio 后端、会话沙盒与开发模式等，见下方上游历史记录。

### Added

- 一键安装脚本 `install.sh`：下载云端编译好的预编译包，校验 sha256 后装到 `~/.local`，不需要 root。支持 Linux（x86_64 / ARM64）与 macOS（Apple 芯片 / Intel）
...
<!-- legacy: 以下为上游 miyu-agent 的历史记录（git-cliff 生成），保留原格式，release.py check 不检查 -->

## [miyu-agent 0.6.0] - 2026-09-13
