...
3. `gqy config` →「接入通讯平台」→「腾讯 QQ」（网页端也可以在「控制台 → 平台 → QQ」的「通用」页设置）：
   - 打开「启用」；
   - access_token 填和 NapCat 一样的 token；
   - 把你自己的 QQ 号填进网页端的「主人 QQ 号」：主人权限最高，自动算管理员，私聊里和终端、网页共用同一份记忆和你的用户资料。朋友要管理权限的，加进「管理员」；
...
网页端「控制台 → 平台 → QQ」分「通用 / 私聊 / 群聊 / 消息记录 / 群聊管理」几页。几个常用的小功能都是插件，可以单独开关：

- **资料卡点赞**：在 QQ 上请她给你点赞，她只给提要求的人本人点，每人每天最多 10 个。主人随时能要，其他人要在私聊或群聊点赞名单里。
- **回复贴表情**：她在群里回复某条消息时，偶尔会在那条消息上贴个表情。
- **群聊黑名单**：管理员直接跟她说「别理 xxx 了」「以后所有群都别理他」「先别理他三个小时」，她就不再回那个人，说「可以理他了」就恢复。
- **主动私聊**（默认关）：私聊安静一阵后，她自己判断要不要、什么时候找你聊。只对管理员和私聊白名单里的人，睡觉时间不发，每天有次数上限。
- 特权记忆只在私聊里用，群聊里她不会把你私下说的事讲出来。
