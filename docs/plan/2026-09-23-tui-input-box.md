# 对话中输入框改为圆角框（2026-09-23 方案，09-24 已施工）

**09-24 施工记录**：按本文实施。大厅（开屏）也用同一个圆角框。框内每行 4 列装饰（`│ ❯ ` / `│   `）、右边 2 列 ` │`，常量与画法在 `cli/repl/layout.rs`（`INPUT_BOX_TEXT_INDENT`、`input_box_edge`、`input_box_row`）。回显 `❯ 消息` + 整行底色 `ECHO_BG_STYLE`，上下各空一行（与原竖条版行数一致）。底栏、候选行、快捷键提示行改为缩进两格。窄终端退回竖条那条没做：框在 20 列以下仍能画（`input_box_wrap_cols` 保底），实测再说。沙箱实测：大厅框、中英混排折行、右框线对齐、空输入光标位置都对。

来由：用户问「TUI 是否摆脱了 opencode 的形式」。结论是没有，09-21 那轮只改了颜色，左竖条 `┃`、平铺正文、底栏这套结构仍是 opencode 一路。用户提出对话中的输入框换成 Claude Code 那种，09-23 在三种形态里选了**完整圆角框**（另两种是上下横线式、只换输入框保留竖条回显）。

本文只记方案。按用户要求，功能性调整先成文档，不干扰同期的发版改造。

---

## 一、已拍板

| 事项 | 决定 |
|---|---|
| 对话中的输入框 | 完整圆角框 `╭─╮ │ ❯ … │ ╰─╯`，底栏在框下方 |
| 开屏大厅（空会话） | ~~不变~~ 09-23 晚改为 Claude Code 式欢迎框，输入框也用同一个圆角框，见 `2026-09-23-tui-launch-and-lobby.md`。本文里「大厅沿用竖条」的说法全部作废 |
| 推翻的定稿 | `docs/plan/2026-09-10-tui-rewrite.md` §3「用户消息与输入区：蓝色粗体 `┃`」与「无边框盒子（通知是全局唯一允许的边框）」。施工时在那两条后注明被本文取代 |

## 二、细节（09-23 已拍板）

| 事项 | 决定 |
|---|---|
| 框线颜色 | 框线 `FAINT` 淡灰，`❯` 用模式主色（普通蓝 / dev 酒红）。模式区分靠 `❯` 和底栏标签 |
| 你发出的消息在对话记录里 | `❯ 消息` + 整行淡底色，不再画竖条。淡底色是内容色，按深浅底各备一套（加进 `render/style.rs` 的 swatches） |
| 开工时间 | 先放着。等 09-21 TUI 配色与 09-23 发版改造两批验收完再开，避免三批改动叠在一起难以定位问题 |

## 三、现状与改动点

输入区只有一个绘制汇合点：`cli/repl/input.rs::render_repl_input_with_footer`（全屏、inline、大厅都走它；大厅靠 `layout: Some((left, width))` 区分）。

**行数不变是这件事便宜的原因**：现在是「空竖条 / N 行输入 / 空竖条 / 底栏」，改完是「上框线 / N 行输入 / 下框线 / 底栏」，`rendered_rows`、底栏行号、`ensure_repl_space` 的算法都不用动。

| 位置 | 改什么 |
|---|---|
| `cli/repl/layout.rs` | `input_prompt_bar` / `submitted_echo_bar` / `submitted_echo_lines` 拆成两套：大厅沿用竖条，对话中用框（框线 `FAINT`，`❯` 用 `mode_accent`）。新增 `input_box_top(cols)`、`input_box_bottom(cols)`、行前缀 `│ ❯ `（续行 `│   `）、行尾 ` │` |
| `cli/repl/input.rs::render_repl_input_with_footer` | `layout.is_none()` 时画框：首尾两行换成框线，每行输入按可见宽度补空格后接右框线。输入折行宽度用 `cols - 2` 调用现有的 `repl_wrapped_input_rows_for_cols`（右边框留两列），前缀宽度按新前缀算 |
| 光标 | 同一函数末尾与 `tail/frame.rs:359` 的全屏光标计算都按新前缀（4 列）算，行偏移不变 |
| 选区 | `tail/screen/select.rs::decoration_width` 认 `│` 开头为 4 列装饰。`drawn` 里记的是**不带右框线**的那份，复制出来不带 `│` |
| 回显 | `submitted_echo_lines` 改成 `❯ 消息` + 整行淡底色（第二节）。`replace_repl_input_with_user_echo` 跟着换 |
| 其他调用方 | `footer.rs`、`inline_picker.rs`、`cli/mod.rs`（3 处）用 `input_prompt_bar` 算宽度或画前缀，逐个核对是跟框走还是跟大厅走 |
| 空输入提示 | `composer_hint::styled` 的可用宽度改按框内宽度算 |
| 命令候选 | inline 模式下候选行占的是底栏那一行（在框外），不受影响。全屏浮层在框上方，不受影响 |

## 四、风险

- **宽字符对齐**：右框线靠「按显示宽度补空格」对齐。东亚歧义宽度字符（部分符号、emoji 变体）在不同终端里宽度不一，右框线可能错一列。缓解：宽度统一走现有的 `visible_width`（与折行同一套），和正文折行的偏差一致，不新增误差来源。
- **窄终端**：框最少要 `│ ❯ ` + 1 列内容 + ` │` = 7 列。低于 20 列时退回无框的竖条样式。
- **resize**：每帧按当前 `cols` 重画框，老框的残影由现有的整行清除覆盖，要在全屏和 inline 两种模式下都实测拖动窗口。

## 五、测试

- 要改的断言：`src/cli/tests/footer_tail.rs` 8 处、`testkit/tui/run.py` 6 处写死了 `┃`。改成引用 `layout.rs` 的函数，不再写字面量。
- 新增：
  - 框的每一行显示宽度都等于 `cols`（含中文、含多行输入、含空输入提示）。
  - 光标落在 `❯ ` 之后，多行时落在正确行。
  - 选区复制不含 `│` 与 `❯`。
  - 大厅仍是竖条（防止改动漏进大厅）。
  - 低于 20 列退回竖条。
- 截图对照：`testkit/tui/shoot_png.py` 改前 / 改后各一套，深浅底都看。

## 六、验收要点（给用户照做）

1. 开 `gqy`，大厅画面和现在一样。
2. 发一条消息，输入框变成圆角框贴底，底栏在框下方。
3. 打多行（Shift+Enter）、打中文长句，右框线始终对齐。
4. 拖选框里的文字复制，粘贴出来不带框线。
5. 拖窄、拉宽窗口，框跟着变，无残影。
6. `gqy dev` 里 `❯` 是酒红。
